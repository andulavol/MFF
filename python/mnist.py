# -*- coding: utf-8 -*-
"""
Created on Sat Nov  6 21:47:54 2021
...
class Dataset:
    ...
    def __init__(self,
        ...
        self.data = self.data.reshape([-1, 28*28]).astype(np.float64)
...
def main(args: argparse.Namespace):
    if args.predict is None:
        ...
        train = Dataset()
        mlp = sklearn.neural_network.MLPClassifier(max_iter=100)
        parameter_space = {
        ...
        clf = sklearn.model_selection.GridSearchCV(mlp, parameter_space, n_jobs=-1, cv=5)
        clf.fit(train.data, train.target) 
        # TODO: Train a model on the given dataset and store it in `model`.
        model = clf.fit(train.data, train.target)
