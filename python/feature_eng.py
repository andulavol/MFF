# -*- coding: utf-8 -*-
"""
Created on Sun Oct 24 00:15:53 2021
...
def main(args: argparse.Namespace) -> tuple[np.ndarray, np.ndarray]:
    ...
    one_hot_encoder = sklearn.preprocessing.OneHotEncoder(handle_unknown="ignore", sparse=False)

    model = sklearn.pipeline.make_pipeline(
        sklearn.preprocessing.StandardScaler(),
        sklearn.compose.ColumnTransformer(
            [
                ("categorical", one_hot_encoder, ["onehot"]),
                ("real-valued", "passthrough", ["numeric"]),
            ]
        ),
    ...
    train_data = model.fit_transform(train_data, train_target)
    test_data = model.fit_transform(test_data, test_target)
